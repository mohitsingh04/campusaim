...
export const createAnswer = async (req, res) => {
  try {
    ...
    if (existingAnswer) {
      return res.status(400).json({
        error:
          "You have already answered this question. You can edit your answer instead.",
      });
    }
  ...
  try {
    ...
    if (!answer) {
      return res.status(404).json({
        error:
          "Answer not found or you are not authorized to edit this answer.",
      });
    }
...
export const deleteAnswer = async (req, res) => {
  try {
    ...
    if (!answer.author.equals(user._id)) {
      return res
        .status(403)
        .json({ error: "You don't have permission to delete this answer." });
    }
