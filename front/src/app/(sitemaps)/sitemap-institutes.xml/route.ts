...
export async function GET() {
  ...
  try {
    const [
      ...
      API.get("/qna", { headers: { origin: baseUrl } }),
    ...
    const mapping: Record<MappedKey, PromiseSettledResult<any>> = {
      gallery: galleryRes,
      hostel: accomodationRes,
      amenities: amenitiesRes,
      teachers: teacherRes,
      faq: faqsRes,
      scholarship: propScholarRes,
      "admission-process": admissionProcessRes,
      "loan-process": loanProcessRes,
  ...
  const propertyRoutes = allProperties.flatMap((property) => {
    const categorySlug = generateSlug(getCategoryById(property.academic_type));
    ...
    const hasDataForTab = {
      ...
      gallery:
        apiData.gallery.filter(
          (g) => String(g.propertyId) === String(property._id),
        ).length > 0,
      hostel:
        apiData.hostel.filter(
      ...
      "admission-process":
        apiData?.["admission-process"]?.filter(
          (f) => String(f.property_id) === String(property._id),
        ).length > 0,
      "loan-process":
        apiData?.["loan-process"]?.filter(
          (f) => String(f.property_id) === String(property._id),
        ).length > 0,
      announcements:
