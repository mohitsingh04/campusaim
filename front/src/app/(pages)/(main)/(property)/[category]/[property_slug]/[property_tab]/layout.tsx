...
export async function generateMetadata({
	...
	if (property?.uniqueId) {
		try {
			const locationRes = await API.get(`/property/location/${property._id}`, {
				headers: { origin: BASE_URL },
			});
...
export default async function PropertyLayout({
	...
	try {
		...
		if (property?.uniqueId) {
			try {
				const reviewRes = await API.get(`/review/property/${property._id}`, {
					headers: { origin: BASE_URL },
				});
		...
		const getCategoryById = (id: string | number) => {
			const cat = categoryRes?.data?.find(
				(item: CategoryProps) => item?._id === id,
			);
		...
		const mainProperty: PropertyProps = {
			...
			category_id: property?.academic_type,
