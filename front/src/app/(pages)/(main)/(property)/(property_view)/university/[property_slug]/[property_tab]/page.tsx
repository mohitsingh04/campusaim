...
import TrafficTracker from "../../../_property_view_components/TrafficTracker";
import EnquiryForm from "../../../_property_view_components/Enquiry";
import RelatedInstitute from "../../../_property_view_components/RelatedInstitutes";
import PropertyJsonSchemaHandler from "../../../_property_view_components/PropertyJsonSchemaHandler";
...
import PropertyTabs from "../../../_property_view_components/PropertyTabs";
...
export async function generateMetadata({
  ...
  const { property_slug, property_tab } = await params;
  ...
  const property_tab_cap =
    property_tab.charAt(0).toUpperCase() + property_tab.slice(1);

  const meta_title = location?.property_city
    ? `${property_tab_cap} | ${property?.property_name} ${location?.property_city}`
    : `${property_tab_cap} | ${property.property_name}`;

  const baseMetaDescription =
    stripHtml(seo?.meta_description || "", 160) ||
    stripHtml(property?.property_description, 160) ||
    `Explore ${property?.property_name} ${location?.property_city} ${property_tab?.replaceAll("-", " ")} including courses, fees, admission & ranking. Top ranking university with top placements & facilities.`;
  const description = NormalizedMetaDesc(
    `${property_tab_cap} ${baseMetaDescription}`,
  );
  ...
  const canonical = `${BASE_URL}/university/${property?.property_slug}/${property_tab}`;
...
export default async function PropertyLayout({
  ...
  try {
    ...
    const heroImageSrc = property?.featured_image?.[0]
      ? `${MEDIA_URL}/${property.featured_image[0]}`
      : DEFAULT_IMAGE;
    const logoImageSrc = property?.property_logo?.[0]
      ? `${MEDIA_URL}/${property.property_logo[0]}`
      : `${BASE_URL}/img/default-images/campusaim-logo.png`;

    const [locationData, reviewData, courseCount, tabexistence]: [
      locationData: PropertyLocationProps | null,
      reviewData: any,
      courseCount: any,
      PropertyTabExistenc: any,
    ] = await Promise.all([
      ...
      property._id ? getPropertyTabExistence(property?._id) : { courses: 0 },
    ]);
