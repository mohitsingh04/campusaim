...
export default function Navbar() {
  ...
  const menuItems = [
    ...
    //   name: "Retreats",
    //   href: "#",
    //   dropdownContent: retreatMenuData,
    //   external: false,
    // },
    { name: "Events", href: "/events", external: false },
    // { name: "Career", href: "#" ,external:false},
    {
      name: "Ask",
  ...
  const handleDesktopMouseEnter = (item: MenuItem) => {
    if (item.dropdownContent) {
      ...
      if (item.name === "Yoga Institutes") {
        const firstKey = Object.keys(item.dropdownContent)[0];
        setActiveDesktopSubMenu(firstKey);
      }
    }
  };
  ...
  if (propertyLoading && courseLoading) return <NavbarLoader />;
  return (
    <>
                  ...
                  >
                    ...
                    {hoveredMenu === item.name && item.dropdownContent && (
                      <div className="absolute top-full left-0 right-0 bg-(--primary-bg) shadow-custom border-t border-(--border) transition-all duration-300 ease-in-out">
                        ...
                        {item.name === "Yoga Institutes" && (
                          <div className="flex">
                            {/* Left sub-menu */}
                            <div className="w-1/4 bg-(--main-emphasis) text-(--text-color-emphasis)">
                              <ul className="py-4">
                                {Object.keys(item.dropdownContent).map(
                                  (subMenuName) => (
                                    <li
                                      key={subMenuName}
                                      onMouseEnter={() =>
                                        setActiveDesktopSubMenu(subMenuName)
                                      }
                                      className={`px-5 py-3 text-(--white) cursor-pointer text-sm ${
                                        activeDesktopSubMenu === subMenuName
                                          ? "relative font-semibold after:content-[''] after:block after:absolute after:top-1/2 after:-translate-y-1/2 after:left-full after:border-y-8 after:border-l-8 after:border-y-transparent after:border-l-(--main-emphasis)"
                                          : "font-medium"
                                      }`}
                                    >
                                      {subMenuName}
                            ...
                            <div className="w-3/4 p-8 max-h-[500px] overflow-y-auto">
                              ...
                              item.dropdownContent[activeDesktopSubMenu] ? (
