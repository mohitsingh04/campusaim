...
export default function EnquiryView() {
  ...
  const hiddenFields = ["_id", "__v", "property_id", "updatedAt"];

  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await API.get(`enquiry/${objectId}`);
        setData(res.data);
      } catch (error) {
  ...
  if (!data) {
    return (
      <div className="flex items-center justify-center h-screen text-[var(--yp-text-primary)]">
        No data found.
      </div>
    );
  }
