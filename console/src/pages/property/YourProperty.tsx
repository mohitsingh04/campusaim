  ...
  const getAllProperties = useCallback(async () => {
    ...
    try {
      ...
      const finalData = properties
        ?.filter((item: PropertyProps) => item?.userId === authUser?._id)
        ?.map((item: PropertyProps) => {
          ...
          const matchedCategory = categories?.find(
            (cat: any) => Number(cat?.uniqueId) === Number(item?.category)
          );

          return {
            _id: item?._id,
            rank: matchedRank?.rank ?? null, // ✅ safe default
    ...
    <div className="space-y-6">
      <Breadcrumbs
        ...
        breadcrumbs={[
          ...
          { label: "Properties", path: "/dashboard/property" },
          { label: "Your" },
        ]}
        extraButtons={[
          {
