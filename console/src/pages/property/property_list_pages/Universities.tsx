...
export function UniversitiesList() {
  ...
  const getAllProperties = useCallback(async () => {
    ...
    try {
      const [propertyRes, locationRes, scoreRes] = await Promise.allSettled([
        API.get("/property"),
        API.get("/locations"),
      ...
      const finalData = properties
        ?.map((item: PropertyProps) => {
          ...
          const matchedScore = scores.find(
            (sc: any) => String(sc?.property_id) === String(item?._id),
          );
          const matchedCategory = categories?.find(
            (cat: any) => String(cat?._id) === String(item?.academic_type),
          );

          const matchedPropertyType = categories?.find(
            (cat: any) => String(cat?._id) === String(item?.property_type),
          );

          if (
            matchedCategory?.category_name !== "University" &&
            matchedCategory?.category_name !== "College"
          ) {
            return null;
          }
