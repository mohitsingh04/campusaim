...
interface AddNewCertificationsProps {
  property: any;
  getCertifications: () => void;
  setAddingMore: (mode: "view" | "add" | "remove") => void;
}

export default function AddNewCertifications({
  property,
  getCertifications,
  setAddingMore,
}: AddNewCertificationsProps) {
    ...
    onSubmit: async () => {
      ...
      try {
        ...
        const response = await API.post(
          `/certifications/add/${property?.uniqueId}`,
          formData
        );

        toast.success(
          response.data.message || "New certifications uploaded successfully."
        );
        setImages([]);
        setPreviews([]);
        setAddingMore("view");
  ...
  return (
    <div>
      {/* Header */}
      <div className="flex items-center justify-between px-2 py-1 border-b border-[var(--yp-border-primary)]">
        <h2 className="font-semibold text-lg text-[var(--yp-text-primary)]">
          Add More Certifications
        </h2>
        <button
          onClick={() => setAddingMore("view")}
          className="text-[var(--yp-red-text)] bg-[var(--yp-red-bg)] px-2 py-2 rounded-lg text-sm font-medium"
        >
          Cancel
        </button>
      </div>
        ...
        >
          ...
          <p className="text-[var(--yp-muted)]">
            {isDragActive
              ...
              : "Drag & drop more achievement images here, or click to select"}
          </p>
        </div>
        ...
        {previews.length > 0 && (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-4">
            {previews.map((file: any, idx: number) => (
              <div
                key={idx}
                className="relative border border-[var(--yp-border-primary)] rounded-lg shadow-sm overflow-hidden"
              >
                <img
                  src={file.preview}
                  alt="preview"
