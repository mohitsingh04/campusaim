...
export default function EditCourseForm({
	...
	const formik = useFormik({
		initialValues: {
			...
			course_short_name: isEditing?.course_short_name || "",
			course_type: normalizeId(
				isEditing?.course_type_id ?? isEditing?.course_type,
			),
			degree_type: normalizeId(
				isEditing?.degree_type_id ?? isEditing?.degree_type,
			),
			program_type: isEditing?.program_type || "",
			best_for: isEditing?.best_for || [],
			course_eligibility: isEditing?.course_eligibility || [],
			duration_value: isEditing?.duration?.split(" ")?.[0] || "",
			duration_type: isEditing?.duration?.split(" ")?.[1] || "",
			status: isEditing?.status || "",

			// ✅ THIS IS CRITICAL
			specialization_fees:
				isEditing?.specialization_fees?.map((s: any) => ({
					specialization_id:
						s.specialization_id?._id || s.specialization_id || "",
					tuition_fee: s.fees?.tuition_fee || "",
					registration_fee: s.fees?.registration_fee || "",
					exam_fee: s.fees?.exam_fee || "",
		...
		onSubmit: async (values) => {
			try {
				const payload = {
					...
					specialization_fees: values.specialization_fees
						.filter((s: any) => s.specialization_id) // ⬅️ IMPORTANT
						.map((s: any) => ({
				...
				await API.patch(`/property-course/${isEditing?._id}`, payload);
	...
	const removeSpecialization = (index: any) => {
		const updated = [...formik.values.specialization_fees];
		updated.splice(index, 1);
		formik.setFieldValue("specialization_fees", updated);
	};

	const selectedSpecializationIds = formik.values.specialization_fees
