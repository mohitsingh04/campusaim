...
export default function EditRetreatForm({
  onUpdate,
  setIsEditing,
  requirements,
  keyoutcomes,
  ...
  const [mainRetreat, setMainRetreat] = useState<RetreatProps | null>(null);
  ...
  const formik = useFormik({
    initialValues: {
      retreat_id: retreatToEdit?.retreat_id || "",
      retreat_short_name:
        retreatToEdit?.retreat_short_name ||
        mainRetreat?.retreat_short_name ||
        "",
      description: retreatToEdit?.description || mainRetreat?.description || "",
      retreat_type:
        retreatToEdit?.retreat_type || mainRetreat?.retreat_type || "",
      retreat_format: retreatToEdit?.retreat_format || "",
      retreat_difficulty_level: retreatToEdit?.retreat_difficulty_level || "",
      retreat_certification_type:
        retreatToEdit?.retreat_certification_type || "",
      capacity: retreatToEdit?.capacity || "",
      cancellation_policy: retreatToEdit?.cancellation_policy || "",
      status: retreatToEdit?.status || "",
      duration_type:
        retreatToEdit?.duration?.split(" ")?.[1] ||
        mainRetreat?.duration?.split(" ")?.[1] ||
        "",
      duration_value:
        retreatToEdit?.duration?.split(" ")?.[0] ||
        mainRetreat?.duration?.split(" ")?.[0] ||
        "",
      certification_available: retreatToEdit?.certification_available || false,
      requirements:
        retreatToEdit?.requirements?.map((item) => ({
          label: getRequirementById(item),
          value: item,
        })) || [],
      key_outcomes:
        retreatToEdit?.key_outcomes?.map((item) => ({
          label: getOutcomesById(item),
          value: item,
        })) || [],
      best_for: [],
      languages:
        retreatToEdit?.languages?.map((item: string) => ({
          label: item,
          value: item,
        })) || [],
      price: [],
      booking_deadline:
        formatDateToFormik(retreatToEdit?.booking_deadline || "") || "",
      start_date: formatDateToFormik(retreatToEdit?.start_date || "") || "",
      end_date: formatDateToFormik(retreatToEdit?.end_date || "") || "",
      accommodation:
        retreatToEdit?.accommodation?.map((item: string) => ({
          label: item,
          value: item,
        })) || [],
      featured_image: null,
      inclusions: retreatToEdit?.inclusions || "",
      exclusions: retreatToEdit?.exclusions || "",
    },
    ...
    onSubmit: async (values) => {
      ...
      try {
        const response = await API.patch(
          `/property-retreat/${retreatToEdit?._id}`,
          formData
        );
        toast.success(response.data.message || "Retreat updated successfully");
        onUpdate();
        setIsEditing(false);
      } catch (error) {
  ...
  useEffect(() => {
    const retreat = getRetreatById(retreatToEdit?.retreat_id) || null;
    setMainRetreat(retreat);
    setBestForList(retreatToEdit?.best_for || []);
    setPriceList(retreatToEdit?.price || {});
    setRoutine(retreatToEdit?.routine || []);
  }, [retreatToEdit, getRetreatById]);
        ...
        <form onSubmit={formik.handleSubmit} className="p-6 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <input
                type="text"
                value={mainRetreat?.retreat_name}
                disabled
            ...
            <div>
              ...
              >
                ...
                {getStatusAccodingToField(status, "Property Retreat").map(
                  (retreat, index) => (
                    <option key={index} value={retreat.parent_status}>
                      {retreat.parent_status}
                    </option>
                  )
                )}
              </select>
