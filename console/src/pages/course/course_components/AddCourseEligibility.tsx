...
export default function AddCourseEligibility({
	onClose,
	isOpen,
	getData,
	courseEligibility,
	...
	const formik = useFormik({
		...
		onSubmit: async (values, { resetForm }) => {
			try {
				...
				toast.success(response.data.message || "Course Eligibility added successfully");
				resetForm();
				getData();
				onClose(false);
			} catch (error) {
	...
	const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
		...
		const matches = courseEligibility.filter((item) =>
			item?.course_eligibility?.toLowerCase()?.includes(query),
		);
		setFiltered(matches);
