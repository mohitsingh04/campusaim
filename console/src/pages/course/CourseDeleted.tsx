...
export function CourseDeleted() {
  ...
  const cardData = getFieldDataSimple(allCourses, "certification_type").map(
    (item, index) => ({
      title: item.title,
      value: item.value,
  ...
  const columns = useMemo<Column<CourseProps>[]>(
    () => [
      ...
      {
        value: (row: CourseProps) => <p>{row?.certification_type}</p>,
        label: "Certification Type",
        key: "certification-type",
        sortingKey: "certification_type",
      },
      {
        value: (row: CourseProps) => (
    ...
    return [
      {
        ...
        filterField: "certification_type" as keyof CourseProps,
