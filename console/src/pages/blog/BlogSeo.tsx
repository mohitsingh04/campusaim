...
export default function BlogSeo() {
  ...
  const [blogSeo, setBlogSeo] = useState<any | null>(null);
  ...
  const getBlogSeo = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const response = await API.get(`/seo/blog/${objectId}`);
  ...
  /** Formik setup */
  const formik = useFormik<BlogSEOFormValues>({
    initialValues: {
      blog_id: blog?._id || "",
      title: blogSeo?.title || blog?.title || "",
      slug: blogSeo?.slug || generateSlug(blog?.title || ""),
      primary_focus_keyword:
        blogSeo?.primary_focus_keyword?.map((k: string) => {
    ...
    <div>
      <Breadcrumbs
        ...
        breadcrumbs={[
          ...
          { label: blog?.title || "", path: `/dashboard/blog/${blog?._id}` },
          { label: "Seo" },
        ]}
      />

      <div className="bg-[var(--yp-primary)] mt-5 rounded-xl shadow-sm">
        <form onSubmit={formik.handleSubmit} className="p-3 md:p-6 space-y-2">
          ...
          <div className="flex flex-col sm:flex-row gap-3 pt-4">
            ...
            >
              {formik.isSubmitting ? "Saving..." : "Save Seo"}
            </button>
          </div>
        </form>
      </div>
