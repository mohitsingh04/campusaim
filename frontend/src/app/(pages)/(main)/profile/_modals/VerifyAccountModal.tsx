...
const VerifyAccountModal = ({
  ...
  const handleVerifyAccountMail = useCallback(async () => {
    setLoading(true);
    try {
      const response = await API.post(
        `/profile/verify-email/email/${profile?.email}`
      );
      if (response) {
        router.push(`/auth/verify-email/send/${profile?.email}`);
      }
    } catch (error) {
      console.log(error);
    } finally {
      ...
      <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full">
        <div className="p-6">
          ...
          <div className="flex gap-3">
            ...
            <button
              onClick={handleVerifyAccountMail}
              className="flex-1 bg-gradient-to-r from-green-500 to-green-600 hover:from-green-600 hover:to-green-700 text-white px-4 py-3 rounded-xl font-medium transition-all duration-200 transform hover:scale-105 shadow-lg hover:shadow-xl disabled:opacity-45"
              disabled={loading}
            >
              Verify Account
            </button>
