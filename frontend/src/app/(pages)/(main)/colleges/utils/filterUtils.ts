...
export const createDynamicFilterOptions = (
	...
	const filteredForAcademicTypes =
		getFilteredInstitutesForCount("academic_type");
	const AcademicTypes = [
		...new Set(
			filteredForAcademicTypes
	...
	return {
		...
		academicTypes: AcademicTypes.map((type) => ({
			name: type,
			value: type,
			count: filteredForAcademicTypes.filter(
				(inst) => generateSlug(inst.academic_type || "") === generateSlug(type)
			).length,
		})),
		ratingCounts,
	};
		...
		const matchesAcademicType =
			...
			(institute.academic_type &&
				filters.academic_type.some(
					(type) =>
						generateSlug(institute.academic_type || "") === generateSlug(type)
				));
